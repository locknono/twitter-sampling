...
def distance(p1, p2):
    ...
    dis = math.sqrt(math.pow(p_diff[0], 2) + math.pow(p_diff[1], 2))
    print(dis)
    return dis


def geodistance(p1, p2):
    y1, x1, y2, x2 = map(math.radians, [p1[1], p1[0], p2[1], p2[0]])
    dlon = y2 - y1
    dlat = x2 - x1
...
def generate_points(kde_function, dataset, oriset, radius, all_well):
    ...
    neighbors_loc = []
    neighbors_id = []
    # step1:Choose a random point in the domain.
    i = int(random.random() * len(dataset))
    ix = dataset[i][0]
    iy = dataset[i][1]
    initial_point = [ix, iy]
    print('initial_point' + str(initial_point))
    samples.append(initial_point)
    print('samples6' + str(samples))
    ...
    while (index < len(dataset)):
        ...
        if (distance(initial_point, [ix, iy]) < tmp_dist_init):
            ...
            del (dataset[index])
        elif (distance(initial_point, [ix, iy]) >= tmp_dist_init) and (
                distance(initial_point, [ix, iy]) <= 2 * tmp_dist_init):
    ...
    while len(dataset) > 0:
        ...
        if len(r2_points) == 0:
            ...
            for point in samples:
                ...
                print('minimum_dist_ps:' + str(minimum_dist_ps))
...
def fun(rate, pds_r):
    pos_file = '../data/finalIDLocation.csv'
    id_arr = []
    ...
    outfile = open("../data/R2sameCir_" + str(rate) + ".json", 'w', newline='')
    outdata = json.dumps(out_arr)
    outfile.write(outdata)
    outfile.close()


pdsR_arr = [130, 90, 65, 50, 30]
