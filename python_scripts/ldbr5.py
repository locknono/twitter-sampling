    ...
    while (samplePoint == None):
        ...
        for p in disks:
            if p.count >= 3:
                ...
                for zp in disks:
                    if zp.count == 0:
                        zeroPointDisks.append(zp)
                zeroRandomTime = 0
                while (samplePoint == None):
                    zeroRandomTime += 1
                    if zeroRandomTime >= 1000:
                        break
                    ...
                    if rp.topic != topic:
                        continue
                    for zp in zeroPointDisks:
                        if getGeoDistance(rp, zp) < zp.r:
                            zp.count += 1
                            zp.isDisk = True
                            samplePoint = rp
                            rp.isDisk = True
                            points.remove(rp)
                            return samplePoint
            if getGeoDistance(randomPoint, p) < p.r:
                p.count += 1
                p.isDisk = True
