...
def drawOneSampleForOneGroup(points: List[Point], topic: int):
    ...
    if len(allFitsPoints) == 0:
        return None
    bannedNumber = 3
    if len(allFitsPoints) == bannedNumber:
        return None
    while (samplePoint == None):
        randomIndices = []
        for i in range(bannedNumber):
            randomIndices.append(random.randint(0, len(allFitsPoints) - 1))
        randomIndex = random.randint(0, len(allFitsPoints) - 1)
        if randomIndex not in randomIndices:
            samplePoint = allFitsPoints[randomIndex]
